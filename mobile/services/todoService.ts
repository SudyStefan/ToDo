...
import { Todo, TodoDraft, TodoDTO, TodoStatus, TodoType } from "../types/todo";
...
class TodoService {
  ...
  private toDTO = (entry: Todo | TodoDraft): TodoDTO => {
    return {
      ...entry,
  ...
  public postTodo = (todo: Todo | TodoDraft): Promise<Todo> => {
    return axios
      .post("http://localhost:4000/todo", this.toDTO(todo))
      .then((res) => {
  ...
  public putTodo = (todo: Todo): Promise<void> => {
    return axios
      .put(`http://localhost:4000/todo/${todo.id}`, this.toDTO(todo))
